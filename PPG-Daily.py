...
# Define required environment variables for PPG-Daily.py
REQUIRED_ENV_VARS = [
    # Plex Server
    "PLEX_URL",
    "PLEX_TOKEN",
...
PLAYLIST_COUNT = int(os.getenv("DAILY_PLAYLIST_COUNT"))
...
def filter_by_minimum_duration(tracks, min_duration_seconds=90):
    ...
    filtered = []
    removed = 0
    # Use progress bar for duration filtering
...
def limit_songs_per_album(playlist_songs, all_available_songs, max_per_album=1):
    ...
    log_info(f"📊 Found {len(albums_to_reduce)} albums that exceed the limit")
    ...
    if songs_needed > 0:
        ...
        if len(available_songs) >= songs_needed:
            ...
            if available_any:
                ...
                filtered_playlist.extend(additional)
                log_info(f"✅ Added {len(additional)} additional songs (some may be from same albums)")
...
def prevent_consecutive_artists(playlist_songs):
    ...
    # Group tracks by artist with progress bar
...
def analyze_artist_distribution(playlist_songs):
    ...
    artist_counts = {}
    # Only show progress bar for larger playlists
...
# Load genre pools from JSON (DAILY_GENRE_GROUPS_FILE — shared with Weekly).
...
def read_daily_log():
    log_debug("Reading daily log...")
    if not os.path.exists(DAILY_LOG_FILE):
        log_debug(f"{DAILY_LOG_FILE} does not exist. Starting with an empty log.")
        return []
    try:
        with open(DAILY_LOG_FILE, "r") as file:
            log_entries = [line.strip() for line in file.readlines()]
            log_debug(f"Daily log loaded: {log_entries}")
            return log_entries
    except Exception as e:
        log_error(f"Error reading daily log: {e}")
        return []


# Write to the daily log file
def write_daily_log(log_entries):
    log_debug("Writing to daily log...")
    try:
        with open(DAILY_LOG_FILE, "w") as file:
            file.writelines(f"{entry}\n" for entry in log_entries)
        log_debug("Daily log updated successfully.")
    except Exception as e:
        log_error(f"Error writing to daily log: {e}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        log_info("🚀 Starting the Daily playlist generation process...")
