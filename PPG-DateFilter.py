...
# Balance artist representation in playlist to max 30% per artist
...
def generate_genre_playlists():
    ...
    # Get liked artists with weekly caching logic (if any playlist needs them)
    liked_artists = set()
    needs_liked_artists = any(group_data.get("prefer_liked_artists", False) for group_data in genre_mixes.values())
    if needs_liked_artists:
    ...
    for i, (genre_group, group_data) in enumerate(genre_mixes.items()):
        ...
        prefer_liked = group_data.get("prefer_liked_artists", False)
        playlist_name = f"{genre_group} Mix"
        print(f"\nStarting generation for Playlist '{playlist_name}'...")
        if prefer_liked:
            print("Preferring songs from liked artists for this playlist")
        ...
        try:
            ...
            if prefer_liked and liked_artists:
